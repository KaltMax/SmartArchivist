  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const totalStorage = docs.reduce((sum, doc) => sum + (doc.fileSize || 0), 0);
        const today = docs.filter((doc) => new Date(doc.uploadDate) >= startOfToday).length;
        const thisWeek = docs.filter((doc) => new Date(doc.uploadDate) >= oneWeekAgo).length;
    ...
    <div className="bg-[#010409] p-6 rounded-lg border border-gray-800 shadow-lg">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-gray-900/40 border border-gray-700 rounded-lg p-4">
          ...
          <div className="text-3xl font-bold text-white">{formatBytes(stats.totalStorage)}</div>
        </div>
        <div className="bg-gray-900/40 border border-gray-700 rounded-lg p-4">
